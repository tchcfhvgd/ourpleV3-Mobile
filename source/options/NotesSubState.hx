...
class NotesSubState extends MusicBeatSubstate
{
	...
	private var grpNumbers:FlxTypedGroup<OptionFlxText>;
	...
	public function new() {
		...
		add(grpNotes);
		grpNumbers = new FlxTypedGroup<OptionFlxText>();
		add(grpNumbers);

		for (i in 0...ClientPrefs.arrowHSV.length) {
			var yPos:Float = (165 * i) + 35;
			for (j in 0...3) {
				var optionText:OptionFlxText = new OptionFlxText(posX + (225 * j) + 250, yPos + 60, Std.string(ClientPrefs.arrowHSV[i][j]));
		...
		hsbText = new OptionFlxText(posX + 225, 0, "Hue          Saturation  Brightness");
		hsbText.size = 32;
		// hsbText.scaleX = 0.6;
		// hsbText.scaleY = 0.6;
		add(hsbText);
